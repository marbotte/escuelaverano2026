---
title: "Actividad: Manejar datos en R"
lang: "es_CL"
output:
  pdf_document:
    toc: true
...
```{r echo=F, message=F,warning=F, purl=FALSE,results='hide'}
library(knitr)
library(kableExtra)
...
```{r}
barplot(table(env_info$HER1Lyon), cex.names=.65, xlab="Hydro-Eco-Región de nivel 1",
        ylab="Número de muestreos")
```
...
* [Clases modos y filtros en R](https://marbotte.github.io/biodata2024/doc_R_clases_modos.html)
...
[Este documento](https://marbotte.github.io/biodata2024/doc_R_buscarRelacionar.html) muestra como funcionan las funciones `which` y `match` que son útil para este ejercicio.

[Este documento](https://marbotte.github.io/biodata2024/doc_R_structuraControl.html) tiene información sobre bucles.
...
**No modifiquen el archivo excel, todo se puede hacer en R**, es al enfrentarse con este tipo de dificultades que nos mejoremos en R.
...
bbScale <-data.frame(
  ...
  description= c("Menos del 1% de cobertura, 3-5 individuos",
                 "Menos del 5% de cobertura, pocos individuos",
                 "~5% más individuos","5%-25%","25%-50%","50%-75%","75%-100%"),
...
mat_bog_chinga <- matrix(c(3000,100,1000,0,0,50,0,200),
                         nrow=2,
                         dimnames=list(c("Bogotá","Chingaza"),
                                              c("Canis lupus",
                                                "Felis catus",
                                                "Tramarctos ornatus",
...
* añadir todas las opciones que les parecen útil (un ejercicio más tarde consistirá en publicar esas funciones en la forma de un paquete R en GitHub)

Construir una función que permita utilizar el formato "base de datos" y hacer la suma de todos las filas que tienen el mismo taxón en la misma unidad de muestreo, y utilizarla para construir la matriz de genero de diatomeas (ver ejercicio previo).
